...
def is_move_ok(grid, x_from, y_from, x_to, y_to):
    ...
    """
    # return ((grid.in_bounds(x_to, y_to)) and (grid.get(x_to, y_to) == None) and (grid.get(x_to, y_to - 1) != "r"))

    if (grid.in_bounds(x_to, y_to)) and (grid.get(x_to, y_to) == None):
        if (x_to, y_to) == (x_from, y_from + 1):
            return True
        elif ((((x_to, y_to) == (x_from - 1, y_from + 1)) or ((x_to, y_to) == (x_from + 1, y_from + 1))) and (grid.get(x_to, y_to - 1) == None)):
            return True
        else:
            return False
...
def do_move(grid, x_from, y_from, x_to, y_to):
    ...
    """
    if is_move_ok(grid, x_from, y_from, x_to, y_to):
